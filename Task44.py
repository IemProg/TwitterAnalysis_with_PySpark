...
result_pdf = spark.read.json(file_name2)                       #.repartition(4).persist()
result_pdf = result_pdf.select('id','geo','timestamp_ms', 'retweeted', 'text', 'created_at',
                              'retweet_count','reply_count')
result_pdf.printSchema()

# Create a new DataFrame that contains young users only
result_pdf = result_pdf.filter( (13 < f.split(f.split(f.col('created_at'), ' ')[3], ":")[0]) \
                            & (f.split(f.split(f.col('created_at'), ' ')[3], ":")[0] <= 18))
